...
function render(){
  ...
  if(box) box.innerHTML=cart.length ? cart.map(i=>{ const p=products.find(x=>x.id===i.product_id); if(!p)return ''; const pricing=productPricing(p); const sub=pricing.sale*i.qty; return '<div class="cartrow"><div><b>'+escapeHtml(p.name)+'</b><br><small>'+(pricing.discounted?'<s>'+rupiah(pricing.normal)+'</s> → ':'')+rupiah(pricing.sale)+' × '+i.qty+' = '+rupiah(sub)+'</small></div><div class="qty"><button type="button" class="cart-qty-btn" onclick="event.preventDefault();event.stopPropagation();minus('+i.product_id+')">−</button><b>'+i.qty+'</b><button type="button" class="cart-qty-btn" onclick="event.preventDefault();event.stopPropagation();plus('+i.product_id+')">+</button></div></div>'; }).join('') : '<div class="cart-empty-state"><span class="cart-empty-icon">＋</span><strong>Keranjangmu masih kosong</strong><small>Pilih menu favoritmu, lalu kembali ke sini untuk lanjut checkout.</small></div>';
  const checkoutPanel=document.querySelector('#cart .checkout-panel');
  if(checkoutPanel) checkoutPanel.hidden=!cart.length;
  const t=totals();
