...
function msg(t){document.getElementById('msg').innerHTML=t?'<div class="error">'+esc(t)+'</div>':'';}
async function init(){
 try{
...
async function load(uid){
 ...
 document.getElementById('statusTimeline').innerHTML=orderStates.map((s,i)=>'<div class="cartrow"><span>'+(i<=current?'✓':'○')+' '+labels[s]+'</span><small>'+((i===current)?'Saat ini':'')+'</small></div>').join('');
