-- Payment provider selection is server-controlled.
-- Customer cannot choose the provider; operations select it through settings.
insert into public.settings(key, value)
values ('payment_provider', 'btn')
...
begin
  ...
  select lower(btrim(value)) into v_provider
  from public.settings where key='payment_provider';

  v_provider := coalesce(nullif(v_provider,''),'btn');
  if v_provider not in ('btn','gateway') then
    raise exception 'Unsupported payment provider configuration';
  end if;
  ...
  v_order_id := private.create_delivery_order(
    ...
    p_items,v_provider,'qris',now()+interval '30 minutes'
  );
  return v_order_id;
end;
$$;

comment on function public.create_customer_delivery_order is
'V1 customer checkout: provider is selected server-side from settings; customer cannot choose payment provider.';
