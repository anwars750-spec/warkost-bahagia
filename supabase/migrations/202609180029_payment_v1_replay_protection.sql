drop index if exists public.payment_webhook_events_provider_reference_uidx;
drop index if exists public.payment_webhook_events_provider_transaction_uidx;

create index if not exists payment_webhook_events_provider_reference_idx
on public.payment_webhook_events(provider, provider_reference)
where provider_reference is not null;

create index if not exists payment_webhook_events_provider_transaction_idx
on public.payment_webhook_events(provider, transaction_reference)
where transaction_reference is not null;

-- Concurrency-safe replay protection: serialize validation by provider/reference.
-- Do not make provider_reference unique at the inbox level because legitimate
-- duplicate events must be recorded and classified as duplicates.
...
begin
  ...
  where provider=v_event.provider
    and provider_reference=v_event.provider_reference
    and id<>v_event.id
