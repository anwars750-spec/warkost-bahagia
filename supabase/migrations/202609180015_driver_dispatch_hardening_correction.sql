-- V1 hardening correction: profiles uses is_active.
...
begin
  ...
  ) then raise exception 'Order is not ready for pickup'; end if;

  if exists (
    select 1 from public.delivery_stops where order_id=v_do.order_id
  ) then raise exception 'Order is already assigned to a delivery stop'; end if;
  ...
  if v_trip_id is null then
    ...
    set active_trip_id=v_trip_id,status='busy',updated_at=now()
